    ...
    return(
        ...
        <NavbarBs className='border-bottom border-secondary'>
            <NavbarBs.Collapse className='justify-content-end'>
                <button onClick={handleShow} variant='btn btn-outline-secondary' className='text-white bg-dark'>
                    ({productsCount})<BsCart className='mx-2 p-3'></BsCart>سبد خرید</button>
            </NavbarBs.Collapse>
        </NavbarBs>
            ...
            <Modal.Header>
                <Modal.Body>
                    {productsCount>0 ?(
                        ...
                        {cart.item.map((item)=>(<CartProduct key={item.id} id={item.id} quantity={item.quantity}></CartProduct>))}
